...
不得开启热重载。构建同时生成 `dist/build-info.json`，并把版本号、完整 Git
提交和源码 dirty 状态注入运行时；登录页底部和开发者工具控制台均可核对简短标识。
已登录用户也可在 Agent 页标题下直接核对该标识。
...
- 使用仓库根目录 `scripts/upload_experience.ps1` 上传时，脚本会将微信版本号
  注入构建，并拒绝提交不匹配或源码未提交的产物。
