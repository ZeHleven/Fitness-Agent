...
```powershell
.\scripts\package_cloudbase_backend.ps1 -Version 0.5.0
```

输出文件为 `deploy/cloudbase/fitness-agent-backend-0.5.0.zip`。

`0.5.0` 增加完整但轻量的多步运行时：execution mode 门控、Micro Planner、逐步 Executor、局部 bounded ReAct、一次动态重规划、独立只读证据并行、结构化 execution trace、角色级 deadline 和安全终止动作契约。高置信健康复合意图可由 rules-first 短路，写工具仍未开放。
