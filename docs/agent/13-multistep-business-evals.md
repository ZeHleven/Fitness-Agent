...
随后对初始 Planner 的 15/20/30 秒 deadline 做同日对照。15 秒与 20 秒组分别覆盖三个核心场景各 5 次，共 15 个样本；30 秒复用紧邻改动前的两个三证据场景各 5 次和故障恢复 3 次，共 13 个样本：

- 三组 completed、deterministic 与 hard-gate 均为 100%，每轮工具调用固定为 3。15 秒和 20 秒组各 45 次工具调用、30 次运行时模型调用，均为理论最小；三证据并行与零 Executor 均为 10/10。30 秒组为 39 次工具调用、28 次模型调用，三证据并行与零 Executor 为 9/10。
- 15 秒组低完成率、高完成率和故障恢复 P95 分别约 20.5、21.3、22.1 秒；20 秒组约 26.0、27.1、31.1 秒；30 秒基线约 35.2、36.8、37.1 秒。15 秒在本组样本中没有以更早 fallback 换取业务、工具或快路径退化。
- 原生 Planner 成功数在 15/20/30 秒组分别为 7/15、2/15、5/13，受供应商波动影响并不随阈值单调变化，不能作为延长等待的依据。受限 fallback 的确定性门禁和最终业务结果才是本次决策条件。
- 因此初始 Planner 默认 deadline 从 30 秒下调为 15 秒；Replanner 拆分为独立的 `AGENT_REPLANNER_TIMEOUT_SECONDS=30`，本次不改变已有观察后的安全收口窗口。该结论来自小样本开发观测，不代表生产 P95/P99 SLO。
